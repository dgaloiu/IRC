...
void broadcastToAllChannelMembers(Server *server, Channel &channel, std::string reply)
{
	...
	{
		appendClientBuffer(server, member->second.getClientFd(), reply);
		member++;
	}
}
...
static void changeChannelMode(Server *server, mode_struct mode_infos, int const client_fd)
{
	...
	for (std::vector<std::string>::iterator it = vector_modes.begin(); it != vector_modes.end(); it++)
	{
		...
		if (str.find("i") != std::string::npos)
			inviteChannelMode(server, mode_infos, client_fd, str);
...
static void modeForChannel(Server *server, mode_struct mode_infos, int const client_fd)
{
	...
	{
		appendClientBuffer(server, client_fd, ERR_NOSUCHCHANNEL(it_client->second.getNickname(), mode_infos.target));
		return;
	}
	...
	if (mode_infos.mode.empty() == true)
	{
		...
		if (it_channel_target->second.getClientList().find(it_client->second.getNickname()) != it_channel_target->second.getClientList().end() && it_channel_target->second.getChannelPassword().empty() == false)
			appendClientBuffer(server, client_fd, RPL_CHANNELMODEISWITHKEY(it_client->second.getNickname(), mode_infos.target, it_channel_target->second.getMode(), it_channel_target->second.getChannelPassword()));
		else
			appendClientBuffer(server, client_fd, RPL_CHANNELMODEIS(it_client->second.getNickname(), mode_infos.target, it_channel_target->second.getMode()));
		return;
	}
	...
	{
		appendClientBuffer(server, client_fd, ERR_CHANOPRIVSNEEDED(it_client->second.getNickname(), mode_infos.target));
		return;
	}
...
static void modeForUser(Server *server, mode_struct mode_infos, int const client_fd)
{
	...
	{
		appendClientBuffer(server, client_fd, ERR_NOSUCHNICK(it_client->second.getNickname(), mode_infos.target));
		return;
	}
	...
	{
		appendClientBuffer(server, client_fd, ERR_USERSDONTMATCH(it_client->second.getNickname()));
		return;
	}

	// If <mode> is not given
	if (mode_infos.mode.empty() == true)
		appendClientBuffer(server, client_fd, RPL_UMODEIS(it_client->second.getNickname(), it_client->second.getMode()));
	...
	if (mode_infos.mode[0] == '+' || mode_infos.mode[0] == '-')
	{
				...
				while (*pos != '+' && *pos != '-' && pos != mode_infos.mode.end())
				{
					if (*pos == 'i')
					{
						if (it_user_target->second.getMode().find("i") == std::string::npos)
						{
							...
							appendClientBuffer(server, client_fd, MODE_USERMSG(it_client->second.getNickname(), "+i"));
						}
					}
					pos++;
				}
				...
				while (*pos != '+' && *pos != '-' && pos != mode_infos.mode.end())
				{
					...
					if (*pos == 'o')
					{
						if (it_user_target->second.getMode().find("o") != std::string::npos)
						{
							...
							appendClientBuffer(server, client_fd, MODE_USERMSG(it_client->second.getNickname(), "-o"));
						}
					}
					pos++;
				}
		...
		if (mode_infos.mode.find("O") != std::string::npos || mode_infos.mode.find("r") != std::string::npos || mode_infos.mode.find("w") != std::string::npos)
			appendClientBuffer(server, client_fd, ERR_UMODEUNKNOWNFLAG(it_client->second.getNickname()));
	}
}
