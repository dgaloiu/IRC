...
void		kill(Server *server, int const client_fd, cmd_struct cmd_infos)
{
	...
	if (killed.empty())
		appendClientBuffer(server, client_fd, ERR_NEEDMOREPARAMS(killer, cmd_infos.name));
	else if (killed_user == NULL)
		appendClientBuffer(server, client_fd, ERR_NOSUCHNICK(killer, killed));
	else if (comment.empty())
		comment = "default";
	else if (client.getMode().find('o') == std::string::npos && isIrcOperator(server, killer) == false)
		appendClientBuffer(server, client_fd, ERR_NOPRIVILEGES(killer));
	else
	{
		...
		appendClientBuffer(server, killed_user->getClientFd(), RPL_KILL(user_id(killer, client.getUsername()), killed, comment));
		...
		appendClientBuffer(server, killed_user->getClientFd(), RPL_ERROR(user_id(killed, killed_user->getUsername()), error_reason));
...
static void	broadcastQuit(Server *server, std::string reply)
{
	...
	{
		appendClientBuffer(server, it_client->second.getClientFd(), reply);
	}
}
