...
static bool hasMinOneChar(std::string str);
...
void part(Server *server, int const client_fd, cmd_struct cmd_infos)
{
	...
	{
		appendClientBuffer(server, client_fd, ERR_NEEDMOREPARAMS(nick, cmd_infos.name));
		return;
	}
	...
	while (hasMinOneChar(cmd_infos.message) == true)
	{
		...
		{
			appendClientBuffer(server, client_fd, ERR_NOSUCHCHANNEL(nick, channel));
			continue;
		}
		else if (it != channels.end() && it->second.doesClientExist(nick) == false) // b) if chan exists and client not in it
		{
			appendClientBuffer(server, client_fd, ERR_NOTONCHANNEL(nick, channel));
			continue;
		}
		else // c) if successful command
		{
			it->second.getClientList().erase(nick);
			appendClientBuffer(server, client_fd, RPL_PART(user_id(nick, client.getUsername()), channel, reason));
...
static void broadcastToAllChannelMembers(Server *server, Channel &channel, std::string user, std::string nick, std::string reason)
{
	...
	{
		appendClientBuffer(server, member->second.getClientFd(), RPL_PART(user_id(nick, user), channel.getName(), reason));
		member++;
	}
}