...
void invite(Server *server, int const client_fd, cmd_struct cmd_infos)
{
	...
	{
		appendClientBuffer(server, client_fd, ERR_USERONCHANNEL(client_nickname, invited_client, channel_name));
		return;
	}

	// If all checks are successful => send a RPL_INVITING + invite to the inviting user
	appendClientBuffer(server, client_fd, RPL_INVITING(user_id(client_nickname, client.getUsername()), client_nickname, invited_client, channel_name));
	...
	for (it = clients.begin(); it != clients.end(); it++)
	{
		...
		{
			appendClientBuffer(server, it->second.getClientFd(),
