...
void nick(Server *server, int const client_fd, cmd_struct cmd_infos)
{
	...
	if (nickname.empty())
	{
		appendClientBuffer(server, client_fd, ERR_NONICKNAMEGIVEN(client.getNickname()));
		if (client.isRegistrationDone() == false)
			client.setNbInfo(-1);
	}
	else if (containsInvalidCharacters(nickname))
	{
		appendClientBuffer(server, client_fd, ERR_ERRONEUSNICKNAME(client.getNickname(), nickname));
		if (client.isRegistrationDone() == false)
			client.setNbInfo(-1);
	}
	else if (isAlreadyUsed(server, client_fd, nickname) == true)
	{
		appendClientBuffer(server, client_fd, ERR_NICKNAMEINUSE(client.getNickname(), nickname));
		if (client.isRegistrationDone() == false)
			client.setNbInfo(-1);
	}
	else
	...
	appendClientBuffer(server, client_fd, RPL_NICK(client.getOldNickname(), client.getUsername(), client.getNickname()));
}
...
bool isAlreadyUsed(Server *server, int client_fd, std::string new_nickname)
{
	...
	{
		if (client->second.getClientFd() != client_fd && client->second.getNickname() == new_nickname)
			return (true);
		client++;
	}
	return (false);
