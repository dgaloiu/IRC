...
void oper(Server *server, int const client_fd, cmd_struct cmd_infos)
{
	...
	{
		appendClientBuffer(server, client_fd, ERR_NOOPERHOST(client.getNickname()));
	}
	else if (isPasswordValid(server, password, name) == false)
	{
		appendClientBuffer(server, client_fd, ERR_PASSWDMISMATCH(client.getNickname()));
	}
	else
	{
		appendClientBuffer(server, client_fd, RPL_YOUREOPER(client.getNickname()));
		client.addMode("o");
		appendClientBuffer(server, client_fd, MODE_USERMSG(client.getNickname(), "+o"));
	}
}
