...
void topic(Server *server, int const client_fd, cmd_struct cmd_infos)
{
	...
	{
		appendClientBuffer(server, client_fd, ERR_NOSUCHCHANNEL(client_nickname, channel_name));
		return;
	}
	if (channel->second.doesClientExist(client_nickname) == false)
	{
		appendClientBuffer(server, client_fd, ERR_NOTONCHANNEL(client_nickname, channel_name));
		return;
	}
	...
	if (topic.empty()) // Afficher le topic
	{
		if (channel->second.getTopic().empty() == false)
			appendClientBuffer(server, client_fd, RPL_TOPIC(client_nickname, channel_name, channel->second.getTopic()));
		else
			appendClientBuffer(server, client_fd, RPL_NOTOPIC(client_nickname, channel_name));
	}
	else // reattribuer le topic
	{
		if (channel->second.getMode().find('t') != std::string::npos && channel->second.isOperator(client_nickname) == false)
			appendClientBuffer(server, client_fd, ERR_CHANOPRIVSNEEDED(client_nickname, channel_name));
		else
		{
...
static void broadcastToChannel(Server *server, Channel &channel, Client &client, std::string channel_name, std::string topic)
{
	...
	{
		appendClientBuffer(server, member->second.getClientFd(), RPL_TOPIC(client_nickname, channel_name, topic));
		member++;
	}
}
/**
