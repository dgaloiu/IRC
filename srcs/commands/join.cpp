...
bool hasMinOneChar(std::string str);
...
void join(Server *server, int const client_fd, cmd_struct cmd_infos)
{
	...
	if (hasMinOneChar(cmd_infos.message) == false)
		appendClientBuffer(server, client_fd, ERR_NEEDMOREPARAMS(client_nickname, cmd_infos.name));
	while (hasMinOneChar(cmd_infos.message) == true)
	{
		channel_name.clear();
		channel_name = getChannelName(cmd_infos.message);

		// erase le channel  de la string = "#foo,#bar" devient "#,#bar"
		...
		else if (it->second.getMode().find('k') != std::string::npos) // Si channel en mode +k
		{
			...
			{
				appendClientBuffer(server, client_fd, ERR_BADCHANNELKEY(client_nickname, channel_name));
		...
		{
			appendClientBuffer(server, client_fd, ERR_CHANNELISFULL(client_nickname, channel_name));
			continue; // on passe la suite, au prochain channel à ajouter síl y en a un
		}
		// vérifier si le client est invited avant de le join au channel
		if (it_chan->second.getMode().find("i") != std::string::npos && it_chan->second.isInvited(client_nickname) == false)
		{
			appendClientBuffer(server, client_fd, ERR_INVITEONLYCHAN(client_nickname, channel_name));
		}
		else
		{
...
void sendChanInfos(Server *server, Channel &channel, std::string channel_name, Client &client)
{
	...
	{
		appendClientBuffer(server, member->second.getClientFd(), RPL_JOIN(user_id(nick, username), channel_name));
		if (channel.getTopic().empty() == false)
			appendClientBuffer(server, member->second.getClientFd(), RPL_TOPIC(nick, channel_name, channel.getTopic()));

		std::string list_of_members = getListOfMembers(nick, channel);
		std::string symbol = getSymbol(channel);

		appendClientBuffer(server, member->second.getClientFd(), RPL_NAMREPLY(username, symbol, channel_name, list_of_members));
		appendClientBuffer(server, member->second.getClientFd(), RPL_ENDOFNAMES(username, channel_name));
		member++;
	}
}
