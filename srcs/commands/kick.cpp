...
void				kick(Server *server, int const client_fd, cmd_struct cmd_infos)
{
	...
	{
		appendClientBuffer(server, client_fd, ERR_NEEDMOREPARAMS(requester_name, cmd_infos.name));
		// sendServerRpl(client_fd, ERR_NEEDMOREPARAMS(requester_name, cmd_infos.name));
		return ;
	}
	else if (it_chan == channels.end())
	{
		appendClientBuffer(server, client_fd, ERR_NOSUCHCHANNEL(requester_name, channel_name));
	...
	{
		appendClientBuffer(server, client_fd, ERR_NOTONCHANNEL(requester_name, channel_name));
	...
	{
		appendClientBuffer(server, client_fd, ERR_USERNOTINCHANNEL(requester_name, kicked_name, channel_name));
	...
	{
		appendClientBuffer(server, client_fd, ERR_CHANOPRIVSNEEDED(requester_name, channel_name));
...
static void			broadcastToChannel(Server *server, Channel &channel, Client &client, std::string kicked, std::string reason)
{
	...
	{
		appendClientBuffer(server, member->second.getClientFd(), \
