...
static std::string	parseChannelName(std::string msg_to_parse);
...
void	names(Server *server, int const client_fd, cmd_struct cmd_infos)
{
	...
	while (hasMinOneChar(cmd_infos.message) == true)
	{
		...
		channel_to_name = parseChannelName(cmd_infos.message);
		...
		{
			appendClientBuffer(server, client_fd, RPL_ENDOFNAMES(client.getNickname(), channel_to_name));
			continue ;
		}
		...
		if (list_of_members.empty() == false)
			appendClientBuffer(server, client_fd, RPL_NAMREPLY(client.getNickname(), symbol, channel_to_name, list_of_members));
		appendClientBuffer(server, client_fd, RPL_ENDOFNAMES(client.getNickname(), channel_to_name));
	}
