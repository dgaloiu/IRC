...
static void broadcastToChannel(Server *server, int const client_fd, std::map<const int, Client>::iterator it_client, std::map<std::string, Channel>::iterator it_channel, std::string target, std::string msg_to_send)
{
	...
	if (it_channel->second.getMode().find("m") != std::string::npos)
	{
		...
		{
			appendClientBuffer(server, client_fd, ERR_CANNOTSENDTOCHAN(it_client->second.getNickname(), it_channel->second.getName()));
			return;
		}
	}
	...
	while (member != it_channel->second.getClientList().end())
	{
		if (member->second.getClientFd() != client_fd) // preventing from sending the message to the sender
			appendClientBuffer(server, member->second.getClientFd(), RPL_PRIVMSG(it_client->second.getNickname(), it_client->second.getUsername(), target, msg_to_send));
		member++;
	}
}
...
void privmsg(Server *server, int const client_fd, cmd_struct cmd_infos)
{
	...
	{
		appendClientBuffer(server, client_fd, ERR_NOTEXTTOSEND(it_client->second.getNickname()));
		return;
	}
	target = cmd_infos.message.substr(1, delimiter - 1); // s'arrete avant le delimiter
	if (target.empty())
	{
		appendClientBuffer(server, client_fd, ERR_NORECIPIENT(it_client->second.getNickname()));
		return;
	}
	...
	if (target[0] == '#')
	{
		...
		if (it_channel == channel_list.end())
			appendClientBuffer(server, client_fd, ERR_NOSUCHNICK(it_client->second.getNickname(), target));
		else
	...
	else
	{
		...
		else
		{
			if (it_target == client_list.end()) // si le user n'existe pas mais le channel oui (gestion channel actif)
			{
				...
				appendClientBuffer(server, it_target->first, RPL_PRIVMSG(it_client->second.getNickname(), it_client->second.getUsername(), target, msg_to_send));
		}
	}
}
/**
  * @brief PRIVMSG is used to send private messages between users, as well as to
	send messages to channels.  <receiver> is usually the nickname of
