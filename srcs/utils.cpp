...
{
	appendClientBuffer(server, client_fd, RPL_WELCOME(user_id(it->second.getNickname(), it->second.getUsername()), it->second.getNickname()));
	appendClientBuffer(server, client_fd, RPL_YOURHOST(it->second.getNickname(), "FT_IRC_NET", "1.0"));
	appendClientBuffer(server, client_fd, RPL_CREATED(it->second.getNickname(), server->getDatetime()));
	appendClientBuffer(server, client_fd, RPL_MYINFO(it->second.getNickname(), "localhost", "1.1", "io", "kost", "k"));
	appendClientBuffer(server, client_fd, RPL_ISUPPORT(it->second.getNickname(), "CHANNELLEN=32 NICKLEN=9 TOPICLEN=307"));
	...
	if (!data)
	{
		appendClientBuffer(server, client_fd, ERR_NOMOTD(it->second.getNickname()));
		return ;
	}
	else
	{
